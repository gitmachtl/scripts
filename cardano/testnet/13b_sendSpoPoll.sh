...
if [ $# -ge 4 ]; then
			...
			lovelacesToSend="${3^^}";
			nodeName="$(dirname $4)/$(basename $(basename $4 .skey) .node)"; nodeName=${nodeName/#.\//};
		 else
		 cat >&2 <<EOF
Usage:  $(basename $0) <From AddressName> <To AddressName or HASH or '\$adahandle'> <Amount in lovelaces / 'ALL' to send all lovelaces(keep your Assets) / 'ALLFUNDS' to send all funds & assets / 'MIN' to send only minimal ada> <NodePoolName>
 ...
 do
	     ...
	     metafile=${paramValue}; metafileExt=${metafile##*.}
   		...
   		if [ "${metadatum}" == null ] || [ -z "${metadatum##*[!0-9]*}" ] || [ "${metadatum}" -lt 0 ] || [ "${metadatum}" -gt 65535 ]; then
			echo "ERROR - MetaDatum Value '${metadatum}' in '${metafile}' must be in the range of 0..65535!"; exit 1; fi
                metafileParameter="${metafileParameter}--metadata-json-file ${metafile} ";
		metafileList="${metafileList}'${metafile}' "
	...
	#Check it its a MessageComment. Adding it to the JSON array if the length is <= 64 chars
        ...
        #Check if its a transaction encryption passphrase
                ...
                passphrase="${paramValue:5}"; #don't to a trimstring here, because also spaces are a valid passphrase !
...
#check that *.node.skey is present
if ! [[ -f "${nodeName}.node.skey" ]]; then
        echo -e "\n\e[35mERROR - Cannot find '${nodeName}.node.skey' ! Note: Hardware-Wallet Cold Keys are not supported currently. \e[0m"; echo; exit 2;
fi
...
echo -e "\e[0mSending lovelaces from Address\e[32m ${fromAddr}.addr\e[0m to Address\e[32m ${showToAddr}\e[0m:"
echo
...
echo -e "\e[0mDestination Address ${showToAddr}:\e[32m ${sendToAddr} \e[90m"
echo
...
if [[ ! "${metafileList}" == "" ]]; then
	echo -e "\e[0mInclude Metadata-File(s):\e[32m ${metafileList}\e[0m\n";
	else
	echo -e "\n\e[35mERROR - Please attach your SPO-Poll-Answer JSON Metadata file!\n\e[0m"; echo; exit 2;
fi
...
case "${lovelacesToSend}" in
        ...
        "ALLFUNDS" )    #If keyword ALLFUNDS was used, send all lovelaces and all assets to the destination address - rxcnt=1
			...
			if [[ ${totalAssetsCnt} -gt 0 ]]; then	#assets are also send completly over, so display them
				echo -ne "\e[0m   Assets to send to ${showToAddr}: \e[33m "
				for (( tmpCnt=0; tmpCnt<${totalAssetsCnt}; tmpCnt++ ))
                        	do

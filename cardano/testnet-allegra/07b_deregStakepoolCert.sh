...
#Generate Dummy-TxBody file for fee calculation
        ...
        ${cardanocli} ${subCommand} transaction build-raw ${nodeEraParam} ${txInString} --tx-out ${sendToAddr}+0 --invalid-hereafter ${ttl} --fee 0 --certificate ${deregCertFile} --out-file ${txBodyFile}
        checkError "$?"
...
rm ${txBodyFile} 2> /dev/null
${cardanocli} ${subCommand} transaction build-raw ${nodeEraParam} ${txInString} --tx-out ${sendToAddr}+${lovelacesToSend} --invalid-hereafter ${ttl} --fee ${fee} --certificate ${deregCertFile} --out-file ${txBodyFile}
checkError "$?"
cat ${txBodyFile}
echo
echo

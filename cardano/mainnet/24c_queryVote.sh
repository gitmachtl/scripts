								...
								*)			echo -e "\n\e[35mERROR - Unknown network type '${withdrawalsNetwork}' for the Withdrawal KeyHash '${withdrawalsHash}' !\n\e[0m"; exit 1;
								...
								if [[ "${proofWithdrawalAddr}" != "-" ]]; then
									if [[ "${proofWithdrawalAddr}" == *"${withdrawalsAddr}"* ]]; then
										echo -e "\e[0m${iconYes} The Withdrawal-Addr in the govAction is contained in the metadata proof!\e[0m\n";
									else
										echo -e "\e[0m${iconNo} The Withdrawal-Addr in the govAction is not contained in the metadata proof!\e[0m\n";
									fi
								fi

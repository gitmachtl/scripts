...
if ${onlineMode}; then
	...
	echo
        ...
        if [[ ${rewardsEntryCnt} == 0 ]]; then echo -e "${iconNo} \e[91mStaking Address is not on the chain, register it first with script 03b!\e[0m\n"; exit 1;
        else echo -e "${iconYes} \e[0mStaking Address is \e[32mregistered\e[0m on the chain with a deposit of \e[32m${keyDepositFee}\e[0m lovelaces, we continue :-)\n";
        fi

        #Checking about rewards on the stake address
        if [[ ${rewardsAmount} == 0 ]]; then echo -e "${iconNo} \e[0mRewards: \e[91mNo rewards found, good.\e[0m\n";
        else
		echo -e "${iconYes} \e[0mRewards available: \e[32m$(convertToADA ${rewardsAmount}) ADA / ${rewardsAmount} lovelaces\e[0m\n"
		echo -e "\e[33mPlease withdraw them first!\e[0m\n"; exit 1;
        fi
...
echo
echo -e "\e[0mMinimum funds required for de-registration: \e[32m $(convertToADA ${minDeregistrationFund}) ADA / ${minDeregistrationFund} lovelaces \e[90mbecause the stakeAddressDepositFee refund will also pay for it, maybe more needed for assets sending!"
echo
